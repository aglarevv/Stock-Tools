...
| **交易计划** | 止盈止损计算、做多/做空、仓位占比、策略预设、价格区间图、价格笼子 |
| **K线训练** | 500~3000根真实A股K线、现货/合约多空、分仓、止盈止损、EMA20、斐波那契回撤画图、画图工具、训练记录持久化 |
| **仓位管理** | 总资金+比例→开仓股数、A股整手/美股整数股/港股整手、限价开仓 |
| **持仓记录** | 做T分段、分批买入、批量编辑、止盈止损管理、多空标记 |
...
| **研报分析** | PDF/DOCX 研报上传→AI 提取方法论→融合/编写研报 |
| **设置** | 数据库配置、AI 接口、OPML 源管理、API Key 加密存储 |
| **图标系统** | 全局统一 SVG 线稿图标（verticalAlign 对齐）、无 emoji 混用 |

### v3.0.0 更新日志

| 类别 | 说明 |
|------|------|
| 🆕 **K线训练** | 随机A股真实K线（500~3000根可选）、现货/合约多空杠杆、分仓开平、止盈止损自动平仓、EMA20、十字虚线、画图工具栏（斐波那契/趋势线/水平线）、训练记录持久化 |
| 🆕 **仓位管理** | 按总资金×比例计算开仓股数，支持A股整手/美股整数股/港股整手、限价开仓 |
| 🆕 **价格笼子** | 连续竞价±2%限价校验，开仓时自动拦截超限订单 |
| 🆕 **画图工具** | 斐波那契回撤（彩虹色、可编辑层级）、趋势线、水平线、拖拽调整锚点、拖拽放置 |
| 🎨 **SVG 图标** | 全局 emoji 替换为统一 SVG 线稿图标（info/palette/history/dice/lock/candlestick/clipboard-list/pin 等），verticalAlign:middle 对齐 |
| ✨ **止盈止损** | 开仓时/开仓后均可设置TP/SL，多头空头双向触发，持仓表格内可编辑 |
| ✨ **模块缓存** | 跨页面切换不丢失状态（window 缓存 + ref 闭包修复） |
| 🔧 **K线可配置** | 后端自动计算start_date，前端下拉选择500/1000/1500/2000/3000根 |
| 🔧 **版本号** | v3.0.0，SVG 黑色 favicon 替代 PNG |
| 🐛 **Bug 修复** | 训练记录重复保存（同步标记savedRef）、斐波那契层间隔层级无法删除（独立FibEditor子组件）、十字虚线不显示（canvas宽度硬编码修复）、拖拽轨迹不可见（同步ref替代异步state） |
...
│           ├── PositionSizing.jsx # 仓位管理（固定比例开仓）
│           ├── KLineTraining.jsx # K线训练游戏（500~3000根、合约多空、画图工具）
│           ├── FuturesCalc.jsx   # 合约计算器
│           ├── Positions.jsx     # 持仓记录（做T、批量编辑）
