...
var require_db = __commonJS({
  "src/server/db.js"(exports2, module2) {
    "use strict";
    ...
    var mysqlPool = null;
    var sqliteDb = null;
    var sqliteDbPath = null;
    var dbName = null;
    function sqliteSQL(sql) {
      let result = sql.replace(/\s*ENGINE\s*=\s*\w+(\s+DEFAULT\s+CHARSET\s*=\s*\w+(\s+COLLATE\s*=\s*\w+)?)?/gi, "").replace(/\s+CHARACTER\s+SET\s*=?\s*\w+/gi, "").replace(/\s+COLLATE\s*=?\s*\w+/gi, "").replace(/\bBIGINT\s+UNSIGNED\b/gi, "INTEGER").replace(/\bBIGINT\b/gi, "INTEGER").replace(/\bINT\s+UNSIGNED\b/gi, "INTEGER").replace(/\bDECIMAL\s*\(\s*\d+\s*,\s*\d+\s*\)/gi, "REAL").replace(/\bVARCHAR\s*\(\s*\d+\s*\)/gi, "TEXT").replace(/\bDATE\b/gi, "TEXT").replace(/\bTIMESTAMP\b/gi, "TEXT").replace(
        /DEFAULT\s+CURRENT_TIMESTAMP(\s+ON\s+UPDATE\s+CURRENT_TIMESTAMP)?/gi,
        "DEFAULT (datetime('now','localtime'))"
      ).replace(/\bON\s+UPDATE\s+CURRENT_TIMESTAMP\b/gi, "").replace(/`([^`]*)`/g, '"$1"').replace(/DATE_FORMAT\s*\(\s*(\w+)\s*,\s*'%Y-%m-%d'\s*\)/gi, "$1").replace(/,?\s*(UNIQUE\s+)?KEY\s+\w+\s*\([^)]+\)/gi, "").replace(/\bAUTO_INCREMENT\b/gi, "AUTOINCREMENT");
...
ON CONFLICT("${col}") DO UPDATE SET ${updatedUpdate}`;
    ...
    async function initMySQL(config2) {
      ...
      const bootstrap = await mysql.createConnection({
        ...
        connectTimeout: 3e3
        // 3 秒超时，避免 Windows 上长时间卡住
      });
      await bootstrap.query(
        `CREATE DATABASE IF NOT EXISTS \`${config2.database}\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`
      );
      await bootstrap.end();
    ...
    async function initSQLite(config2) {
      ...
      if (process.env.ELECTRON_USER_DATA) {
        ...
        dbDir = path2.join(os2.homedir(), ".stock-toolbox");
      }
      if (!fs2.existsSync(dbDir)) {
        fs2.mkdirSync(dbDir, { recursive: true, mode: 448 });
      }
      sqliteDbPath = path2.join(dbDir, "toolbox.db");
      console.log(`[db] SQLite database: ${sqliteDbPath}`);
      let db2;
      if (fs2.existsSync(sqliteDbPath)) {
        const buffer = fs2.readFileSync(sqliteDbPath);
    ...
    function saveSQLite() {
      ...
      try {
        ...
        fs2.writeFileSync(tmpPath, Buffer.from(data));
        fs2.renameSync(tmpPath, sqliteDbPath);
      } catch (err) {
        console.error("[db] Failed to save SQLite database:", err.message);
      }
    }
    async function initialize(config2) {
      dbName = config2.mysql?.database || "stock_toolbox";
      try {
        console.log("[db] Trying MySQL...");
        mysqlPool = await initMySQL(config2.mysql);
        dbType = "mysql";
      ...
      try {
        ...
        sqliteDb = await initSQLite(config2);
        dbType = "sqlite";
    ...
    async function columnExists(tableName, columnName) {
      if (dbType === "mysql") {
        try {
          const db2 = dbName || "stock_toolbox";
          const [rows] = await mysqlPool.query(
            `SELECT 1 FROM information_schema.columns
         WHERE table_schema = '${db2}' AND table_name = '${tableName}' AND column_name = '${columnName}'
         LIMIT 1`
          );
    ...
    async function addColumnIfNotExists(tableName, columnName, columnDef, fallback) {
      ...
      const sqliteDef = columnDef.replace(/\bVARCHAR\s*\(\d+\)/gi, "TEXT").replace(/\bTEXT\b/i, "TEXT").replace(/\bDECIMAL\s*\(\d+,\d+\)/gi, "REAL");
      const sqliteFallback = typeof fallback === "string" ? `'${fallback.replace(/'/g, "''")}'` : String(fallback ?? "''");
      try {
...
// node_modules/feedparser/lib/constants.js
var require_constants = __commonJS({
  "node_modules/feedparser/lib/constants.js"(exports2, module2) {
    var NAMESPACES = {
    ...
    var HTML_URI_ATTRS = /* @__PURE__ */ new Set([
      "href",
      "src",
      "uri",
      "srcset",
...
var require_utils = __commonJS({
  "node_modules/feedparser/lib/utils.js"(exports2, module2) {
    var { URL: WHATWGURL } = require("url");
    var { NAMESPACES, HTML_URI_ATTRS, HTML_TAGS } = require_constants();
    var _get = require_lodash();
    ...
    function readMarkupAt(str, i) {
      ...
      while (j < str.length) {
        ...
        if (quote) {
          ...
          return { tagName, isClosing, len: j + 1 - i };
        }
        j++;
      }
      return null;
...
// node_modules/feedparser/lib/feedparser.js
var require_feedparser = __commonJS({
  "node_modules/feedparser/lib/feedparser.js"(exports2, module2) {
    var sax = require_sax();
    var addressparser = require_addressparser();
    var indexOfObject = require_array_indexofobject();
    var { inherits } = require("util");
    var { Transform: TransformStream } = require_readable();
    var { HTML_URI_ATTRS } = require_constants();
    var _ = {
      ...require_utils(),
      has: require_lodash2(),
      assign: require_lodash3(),
      uniq: require_lodash4()
    };
    ...
    FeedParser.prototype.handleCloseTag = function(el) {
      var node = {
        ...
        "#prefix": "",
        "#local": ""
      }, stdEl, item, base, baseurl, isIllegallyNested = false;
      ...
      if (node["#name"] === "item" || node["#name"] === "entry" || node["#local"] === "item" && (node["#prefix"] === "" || node["#type"] === "rdf") || node["#local"] == "entry" && (node["#prefix"] === "" || node["#type"] === "atom")) {
        ...
        this.push(item);
      } else if (node["#name"] === "channel" || node["#name"] === "feed" || node["#local"] === "channel" && (node["#prefix"] === "" || node["#type"] === "rdf") || node["#local"] === "feed" && (node["#prefix"] === "" || node["#type"] === "atom")) {
    ...
    FeedParser.prototype.handleMeta = function handleMeta(node, type, options) {
      if (!type || !node) return {};
      var meta = {}, normalize = !options || options && options.normalize;
      if (normalize) {
    ...
    FeedParser.prototype.handleItem = function handleItem(node, type, options) {
      if (!type || !node) return {};
      var item = {}, normalize = !options || options && options.normalize;
      if (normalize) {
    ...
    exports2 = module2.exports = FeedParser;
  }
});
...
var ENC_PASSPHRASE = "stock-toolbox-local-enc-key-2025";
var ENC_SALT = "toolbox-salt";
var ENC_KEY = crypto.pbkdf2Sync(ENC_PASSPHRASE, ENC_SALT, 1e5, 32, "sha256");
var ENC_IV_LEN = 12;
...
var dbReady = initializeDatabase().catch((error) => {
  dbInitError = error;
...
function cleanText(value, fallback = "", maxLength = 2e3) {
...
function buildTradePlan(input) {
  ...
  const tradeNotes = cleanText(input.tradeNotes, "", 2e3);
  return {
    symbol,
    buyPrice,
    shares,
...
function buildDailyReview(input) {
  const reviewDate = /^\d{4}-\d{2}-\d{2}$/.test(String(input.reviewDate || "")) ? String(input.reviewDate) : todayDateString();
  const symbol = cleanText(input.symbol, "\u81EA\u9009\u80A1", 64);
  const holdingStyle = cleanText(input.holdingStyle, "\u77ED\u7EBF", 64);
  const buySignal = cleanText(input.buySignal, "", 2e3);
  const sellSignal = cleanText(input.sellSignal, "", 2e3);
  const buyPrice = Math.max(0, toFiniteNumber(input.buyPrice, "buyPrice"));
  ...
  const text = (key) => cleanText(input[key], "", 5e3);
  return {
    reviewDate,
    symbol,
    buySignal,
...
async function handleApi(request, response, url) {
  ...
  if (request.method === "GET" && url.pathname === "/api/health") {
    ...
    sendJson(response, dbStatus === "error" ? 503 : 200, {
      ...
      message: dbInitError ? `\u6570\u636E\u5E93\u9519\u8BEF\uFF1A${dbInitError.message}` : dbInitialized ? `\u6570\u636E\u5E93\u5C31\u7EEA (${db.getDbType()})` : "\u6570\u636E\u5E93\u521D\u59CB\u5316\u4E2D..."
    });
    return;
  }
  try {
    ...
    try {
      ...
      const [result] = await db.execute(
        ...
        [
          ...
          review.pnlAmount,
          review.pnlRate,
          review.indexJudgment,
          review.volumeJudgment,
          review.sentimentJudgment,
          review.capitalDirection,
          review.leadingSectors,
          review.laggingSectors,
          review.sustainability,
          review.stockStrength,
          review.volAmpRanking,
          review.limitAnalysis,
          review.operationReason,
          review.profitAttribution,
          review.lossAttribution,
          review.executionNotes,
          review.improvementPlan,
          review.marketPlan,
          review.positionPlan,
          review.newCandidates
        ]
      );
  ...
  if (request.method === "GET" && url.pathname === "/api/dashboard") {
    try {
      ...
      sendJson(response, 200, {
        ...
        winRate: Number(winCount) + Number(lossCount) > 0 ? Math.round(Number(winCount) / (Number(winCount) + Number(lossCount)) * 100) : 0,
  ...
  if (request.method === "POST" && url.pathname === "/api/ai/chat") {
    try {
      ...
      if (reviewData && typeof reviewData === "object") {
        ...
        if (r.buyPrice !== void 0 && r.buyPrice !== null && r.buyPrice !== "") parts.push(`\u4E70\u5165\u4EF7\uFF1A${r.buyPrice}`);
        if (r.sellPrice !== null && r.sellPrice !== void 0 && r.sellPrice !== "") parts.push(`\u5356\u51FA\u4EF7\uFF1A${r.sellPrice}`);
        if (r.shares) parts.push(`\u80A1\u6570\uFF1A${r.shares}`);
        if (r.pnlAmount !== void 0 && r.pnlAmount !== null) parts.push(`\u76C8\u4E8F\u91D1\u989D\uFF1A${r.pnlAmount}`);
        if (r.pnlRate !== void 0 && r.pnlRate !== null && r.pnlRate !== "") parts.push(`\u6536\u76CA\u7387\uFF1A${r.pnlRate}%`);
        const sections = [];
  ...
  if (request.method === "POST" && url.pathname === "/api/ai/parse-file") {
    try {
      ...
      if (ext === ".txt" || ext === ".md" || mimeType === "text/plain" || mimeType === "text/markdown") {
        text = buffer.toString("utf-8").slice(0, 5e4);
      } else if (ext === ".docx" || mimeType === "application/vnd.openxmlformats-officedocument.wordprocessingml.document") {
        try {
          const mammoth = require("mammoth");
          const result = await mammoth.extractRawText({ buffer });
          text = result.value.slice(0, 5e4);
        } catch (e) {
  ...
  if (request.method === "GET" && url.pathname === "/api/daily-digest") {
    try {
      ...
      sendJson(response, 200, {
        ...
        articles: (() => {
          try {
            return JSON.parse(d.articlesJson || "[]");
          } catch {
            return [];
          }
        })()
...
`;
      ...
      let idx = 0;
      const sop1Text = buildSection("SOP 1 \u6DF1\u5EA6\u6D1E\u5BDF \u2014 \u957F\u671F\u903B\u8F91\xB7\u5546\u4E1A\u6A21\u5F0F\xB7\u5B8F\u89C2\u5B9A\u8C03\uFF08\u6295\u8D44\u7684 Why \u548C What\uFF09", sopGroups.sop1, idx);
      idx += sopGroups.sop1.length;
      const sop2Text = buildSection("SOP 2 \u52BF\u80FD\u626B\u63CF \u2014 \u8D44\u91D1\u6D41\u5411\xB7\u8BC4\u7EA7\u8C03\u6574\xB7\u7A81\u53D1\u70ED\u70B9\uFF08\u6295\u8D44\u7684 When \u548C Where\uFF09", sopGroups.sop2, idx);
      idx += sopGroups.sop2.length;
      const sop3Text = buildSection("SOP 3 \u533A\u57DF/\u5782\u7C7B \u2014 \u67E5\u6F0F\u8865\u7F3A\xB7\u7279\u5B9A\u673A\u4F1A\xB7\u6280\u672F\u524D\u6CBF\uFF08\u6295\u8D44\u7684 How \u548C Next\uFF09", sopGroups.sop3, idx);
...
var _feedparser = null;
var _nodeFetch = null;
function getFeedParser() {
  if (!_feedparser) _feedparser = require_feedparser2();
  return _feedparser;
}
function getNodeFetch() {
  if (!_nodeFetch) _nodeFetch = require_lib2();
  return _nodeFetch;
}
var newsCache = { date: "", data: null };
...
async function generateDailyDigest() {
  ...
  const results = await Promise.allSettled(
    feeds.map(
      (feed) => fetchRSS(feed.xmlUrl).then((items) => ({ feed, items }))
    )
  );
  ...
  const digest = {
    ...
    allArticles: allFlat,
    marketSentiment: assessMarketSentiment(articles),
    topics
  };
...
function fetchRSS(url) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      fetch2(url, { timeout: 5e3, headers: { "User-Agent": "Toolbox/2.0" } }).then((res) => {
        if (!res.ok) {
...
function classifyArticles(articles) {
  ...
  return ordered.map(([name, items]) => ({
    name,
    count: items.length,
    highlights: items.slice(0, 8).map((a) => ({
      title: a.title,
      source: a.source,
      link: a.link,
      summary: a.summary || "",
...
function assessMarketSentiment(articles) {
  ...
  if (bullish > bearish * 1.5) return { label: "\u504F\u4E50\u89C2 \u{1F60A}", ratio: `${bullish}:${bearish}`, detail: `\u5229\u597D ${bullish} \u6761 vs \u5229\u7A7A ${bearish} \u6761\uFF0C\u5E02\u573A\u60C5\u7EEA\u504F\u6696` };
  if (bearish > bullish * 1.5) return { label: "\u504F\u60B2\u89C2 \u{1F61F}", ratio: `${bullish}:${bearish}`, detail: `\u5229\u7A7A ${bearish} \u6761 vs \u5229\u597D ${bullish} \u6761\uFF0C\u6CE8\u610F\u98CE\u9669` };
  return { label: "\u4E2D\u6027 \u{1F610}", ratio: `${bullish}:${bearish}`, detail: `\u5229\u597D ${bullish} \u6761\uFF0C\u5229\u7A7A ${bearish} \u6761\uFF0C\u591A\u7A7A\u5747\u8861` };
}
